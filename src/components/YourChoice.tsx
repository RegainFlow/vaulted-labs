...
const options = [
  ...
  },
  {
    ...
    accentBg: "bg-white",
    glowClass: "hover:shadow-[0_0_20px_rgba(255,255,255,0.1)]",
  ...
  return (
    <section className="border-y border-border bg-surface px-4 py-20 sm:py-28">
      <div className="mx-auto max-w-6xl">
        <h2 className="mb-4 text-center text-3xl font-bold sm:text-4xl uppercase tracking-tighter italic">
          Your Item, <span className="text-accent">Your Rules</span>
        </h2>
        <p className="mx-auto mb-14 max-w-lg text-center text-text-muted">
          Every reveal comes with three primary liquidity options.
        </p>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3 lg:gap-8">
          {options.map((opt, i) => (
            <motion.div
              ...
              className={`rounded-2xl border border-white/5 bg-bg p-6 transition-all lg:p-8 ${opt.hoverBorder} ${opt.glowClass}`}
            >
              <div className={`mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-surface-elevated border border-white/5 ${opt.accentText}`}>
                {opt.icon}
              </div>
              <h3 className={`mb-3 text-xl font-bold uppercase tracking-wider ${opt.accentText}`}>
                {opt.title}
              </h3>
