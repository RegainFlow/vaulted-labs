...
const HUD_TOOLTIPS: Record<
  string,
  { title: string; text: string; color: string; borderColor: string }
> = {
  credits: {
    title: "Credits",
...
export function Navbar({
  ...
  const [activeTooltip, setActiveTooltip] = useState<HudTooltipKey | null>(
    null
  );
  ...
  const xpIntoLevel = Math.max(
    0,
    levelInfo.currentXP - levelInfo.xpForCurrentLevel
  );
  ...
  const xpRemaining = Math.max(
    0,
    levelInfo.xpForNextLevel - levelInfo.currentXP
  );

  const toggleTooltip = useCallback(
    (key: HudTooltipKey, element: HTMLElement) => {
  ...
  const DOCK_COLORS = {
    wallet: {
      active: "border-vault-gold/35 text-vault-gold bg-vault-gold/[0.08]",
      hover: "hover:text-vault-gold",
    },
    vaults: {
      active: "border-accent/35 text-white bg-accent/[0.10]",
      hover: "hover:text-white",
    },
    locker: {
      active: "border-white/14 text-white bg-white/[0.06]",
  ...
  const handleDockClick = (target: "wallet" | "vaults" | "locker") => {
    if (target === "locker") {
      if (!isFeatureUnlocked("locker", xp)) {
        trackEvent(AnalyticsEvents.FEATURE_LOCKED_CLICKED, {
          featureKey: "locker",
        });
  ...
  const desktopHudButtonClass =
    "command-segment flex h-[54px] items-center gap-2 rounded-[16px] px-3 cursor-pointer";
  const mobileHudButtonClass =
    "command-segment flex min-h-[52px] flex-col items-center justify-center rounded-[14px] px-2 py-1.5 text-center cursor-pointer";
        ...
        >
          ...
          >
            <div
              className={
                showHUD
                  ? "w-full md:w-auto flex justify-center md:block"
                  : "shrink-0"
              }
            >
            ...
            {showHUD ? (
              <div
                className="system-rail relative hidden items-center gap-1.5 px-2 py-1.5 md:flex"
                data-tutorial="hud-desktop"
              >
                <button
                  type="button"
                  onClick={(event) =>
                    toggleTooltip("credits", event.currentTarget)
                  }
                  className={`${desktopHudButtonClass} min-w-[132px] justify-between`}
                >
                  <span className="system-kicker whitespace-nowrap">Credits</span>
                  <span
                ...
                <button
                  type="button"
                  onClick={(event) =>
                    toggleTooltip("shards", event.currentTarget)
                  }
                  className={`${desktopHudButtonClass} min-w-[92px] justify-center gap-1.5 px-2.5`}
                >
                  <svg
                    width="11"
                    height="11"
                    viewBox="0 0 24 24"
                    fill="none"
                    className="text-rarity-rare shrink-0"
                  >
                    <path
                  ...
                  <span className="text-[10px] font-mono font-bold text-rarity-rare">
                    {shards}
                  </span>
                ...
                <button
                  type="button"
                  onClick={(event) =>
                    toggleTooltip("level", event.currentTarget)
                  }
                  className={`${desktopHudButtonClass} min-w-[118px] justify-between`}
                >
                  <svg
                    width="11"
                    height="11"
                  ...
                  <div className="min-w-[78px]">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[8px] font-black uppercase tracking-[0.18em] text-accent">
                        Lv {levelInfo.level}
                      </span>
          ...
          {showHUD && (
            <div
              ...
              data-tutorial="hud"
            >
              <div className="system-rail grid w-full grid-cols-3 gap-1 p-1">
                <button
                  type="button"
                  onClick={(event) =>
                    toggleTooltip("credits", event.currentTarget)
                  }
                  className={mobileHudButtonClass}
                >
                  <p className="system-kicker">Credits</p>
                  <p
                    key={cashoutFlashTimestamp}
                ...
                <button
                  ...
                  className={mobileHudButtonClass}
                >
                  <p className="system-kicker">Shards / Spins</p>
                ...
                <button
                  ...
                  className={mobileHudButtonClass}
                >
                  <p className="system-kicker">Level</p>
                  ...
                  <p className="mt-0.5 text-[8px] font-mono text-text-dim">
                    {xpIntoLevel}/{xpNeeded}
                  </p>
                </button>
              </div>
              ...
              return (
                ...
                >
                  <div className="module-card relative border-l-2 px-3.5 py-3">
                    <p
                      className={`mb-1 text-[10px] font-black uppercase tracking-wider ${tip.color}`}
                    >
                      {tip.title}
                    </p>
                    ...
                    {activeTooltip === "level" && (
                      <p className="mt-2 text-[10px] font-mono text-text-dim">
                        {xpIntoLevel}/{xpNeeded} XP in level, {xpRemaining} XP
                        to Lv {levelInfo.level + 1}
                      </p>
                    )}
                  </div>
                </motion.div>
