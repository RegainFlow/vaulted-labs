...
import type { ItemStatus } from "../../types/game";
...
export function InventoryGrid() {
  ...
  const filtered = filter === "all" ? inventory : inventory.filter((item) => item.status === filter);
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        ...
        >
          ...
          <p className="text-text-dim text-xs">Open vaults on the Play page to start collecting.</p>
        </motion.div>
      ) : (
