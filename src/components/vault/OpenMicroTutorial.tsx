...
const STEPS: TutorialStepConfig[] = [
  ...
  },
  {
    id: "bonus",
    title: "Bonus Round",
    message: "Premium vaults can trigger Vault Lock. Match tiers for shards.",
    selector: '[data-tutorial="bonus-round"]'
  },
  {
    id: "reveal",
...
export function OpenMicroTutorial({
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener(OPEN_TUTORIAL_SPIN_STARTED_EVENT, handleSpinStarted);
    };
  }, [isActive, step.id]);
  ...
  useEffect(() => {
    ...
    return () => {
      target.removeEventListener("click", handleSpinClick);
    };
  }, [isActive, step]);
  ...
  }, [isActive, stepIndex, targetRect, targetMissing]);
  ...
  const handleSkip = () => {
    completeTutorial(AnalyticsEvents.TUTORIAL_SKIPPED);
  };
  ...
  const waitingForTargetTap = Boolean(step.advanceOnTargetClick);
  const waitingForSpinTap = step.id === "spin";
  const waitingForBonusComplete = step.id === "bonus" && bonusSeen;
  const hideDuringSpin = step.id === "spin" && spinActionTriggered;

  if (!isActive || hideDuringSpin) return null;
  ...
  const isBonusStep = step.id === "bonus";
  const spotlightBorderClass = isBonusStep ? "border-neon-green" : "border-accent";
  const spotlightShadow = isBonusStep
    ? "0 0 30px rgba(57,255,20,0.3), inset 0 0 18px rgba(57,255,20,0.14)"
    : "0 0 30px rgba(255,45,149,0.35), inset 0 0 18px rgba(255,45,149,0.15)";
  const tooltipContainerClass = isBonusStep
    ? "border-neon-green/45 shadow-[0_0_30px_rgba(57,255,20,0.2)]"
    : "border-accent/40 shadow-[0_0_30px_rgba(255,45,149,0.25)]";
  const waitingTextClass = isBonusStep ? "text-neon-green" : "text-neon-cyan";
  const actionButtonClass = isBonusStep
    ? "bg-neon-green/90 text-bg hover:bg-neon-green"
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {targetRect && (
          <div
            className={`absolute rounded-xl border-2 animate-pulse ${spotlightBorderClass}`}
            style={{
              top: targetRect.top,
        ...
        <motion.div
          ...
          className={`absolute w-[calc(100%-16px)] sm:w-auto max-w-[300px] sm:max-w-sm rounded-xl border bg-surface-elevated p-4 sm:p-5 z-[211] pointer-events-auto overflow-y-auto ${tooltipContainerClass}`}
          style={{
            ...tooltipStyle,
        ...
        >
          ...
          <div className="flex items-center justify-between">
            ...
            {waitingForTargetTap ? (
              <span className={`text-[10px] font-bold uppercase tracking-wider ${waitingTextClass}`}>
                Tap Vault
              </span>
            ) : waitingForSpinTap ? (
              <span className={`text-[10px] font-bold uppercase tracking-wider ${waitingTextClass}`}>
              ...
              <button
                type="button"
                onClick={handleNext}
                className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-colors cursor-pointer ${actionButtonClass}`}
              >
